...
public class PlannerAgent extends Agent {
    ...
    private Stack<StripsAction> AstarSearch(GameState startState) {
    	...
    	Set<GameState> explored = new HashSet<GameState>();
    	GameState current = startState;
    	frontier.add(current);
    	
    	while (!frontier.isEmpty()) {    		
    		current = extractMin(frontier);  //TODO -- implement this
    		frontier.remove(current);
    		
			if (current.isGoal()) {
    			System.out.println("SUCCESS: Optimal plan found.");    			
    			return reconstructPlan(current);  //TODO -- implement this
    		}
			...
			while (i.hasNext()) {			
				...
				if (!explored.contains(child)) {
					...
					if (!frontier.contains(child)) {
						...
						GameState temp = null;
						for(GameState candidate : frontier) {
							if(candidate.equals(child)) {  // they have the same heuristics
								temp = candidate;
							}
						}
